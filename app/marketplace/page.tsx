...
{galleryOpen && (
<div className="fixed inset-0 bg-black/95 z-50 flex items-center justify-center p-4" onClick={() => setGalleryOpen(false)}>
<div className="relative w-full max-w-5xl" onClick={(e) => e.stopPropagation()}>
<button onClick={() => setGalleryOpen(false)} className="absolute -top-12 right-0 text-white text-2xl hover:text-agri-green z-10">✕ Close</button>
<div className="flex gap-4">
<div className="flex-1 flex items-center justify-center">
<div className="relative">
<img src={galleryImages[currentImageIndex]} alt="Product" className="max-w-full max-h-[80vh] object-contain rounded-lg" />
{galleryImages.length > 1 && (
<>
<button onClick={prevImage} className="absolute left-2 top-1/2 -translate-y-1/2 bg-black/70 hover:bg-black/90 text-white px-4 py-8 rounded text-2xl">‹</button>
<button onClick={nextImage} className="absolute right-2 top-1/2 -translate-y-1/2 bg-black/70 hover:bg-black/90 text-white px-4 py-8 rounded text-2xl">›</button>
</>
)}
</div>
</div>
...
<button key={idx} onClick={() => setCurrentImageIndex(idx)} className={`w-full h-20 rounded overflow-hidden border-2 transition ${idx === currentImageIndex ? 'border-agri-green' : 'border-transparent opacity-60 hover:opacity-100'}`}>
...
{galleryImages.length > 1 && (
<div className="text-center mt-4 text-white">{currentImageIndex + 1} / {galleryImages.length}</div>
)}
</div>
</div>
)}
